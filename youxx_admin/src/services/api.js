...
// 请求拦截器：附加 Token
// 登录态用 sessionStorage 按标签页隔离，避免同一浏览器多窗口登录不同账号时互相覆盖
...
export function uploadProductImageApi(formData, categoryId) {
  // 商品图片上传(multipart/form-data)，后端按 categoryId 压缩落盘到 upload_resources/products/{categoryId}/，返回可访问 url。
  // 业务约束：未选定商品分类不允许上传图片，故 categoryId 为必填。
